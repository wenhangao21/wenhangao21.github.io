...
Readers are assumed to be familiar with flow matching: [paper](https://arxiv.org/pdf/2210.02747); [tutorial](https://wenhangao21.github.io/blogs/files/g9_flow_matching/g9_flow_matching/).

The code follows the same style as the flow matching tutorial, using simple if–else statements to switch between flow matching (FM) and mean flow (MF) for easy transition and comparison.

The notebook version with the full code can be accessed [here](https://github.com/wenhangao21/Tutorials/tree/main/Generative_Models/mean_flow). 
